...
with open('all_60_products.html', 'w', encoding='utf-8') as f:
    current_cat = None
    cat_names = {
        'starter': '🎯 СТАРТОВЫЕ НАБОРЫ',
        'premium': '🚀 ПРЕМИУМ НАБОРЫ',
        'vip': '⭐ ПРЕМИУМ ФУНКЦИИ',
        'limited': '🔮 QUANHASH НАБОРЫ',
        'ultra': '🎁 КОМБО СЕТЫ',
        'mega': '👑 VIP ФУНКЦИИ',
    }
    ...
    for p in products:
        if current_cat != p['cat']:
            f.write(f'                        <div data-category="{p["cat"]}" style="display:block;text-align:center;font-size:13px;font-weight:700;color:#fff;margin:8px 0 4px;">{cat_names[p["cat"]]}</div>\n\n')
            current_cat = p['cat']
        ...
        f.write(f'''                        <div data-category="{p['cat']}" style="background:rgba({colors['color']},0.25);padding:12px;border-radius:10px;border:1px solid rgba({colors['color']},0.5);">
                            <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
                                <div style="font-size:32px;">{p['emoji']}</div>
                                <div style="flex:1;">
                                    <div style="font-size:15px;font-weight:700;color:#fff;">{p['title']}</div>
...
print("✅ HTML для всех 60 карточек сохранён в all_60_products.html")
